...
class MetadataServer:
    def __init__(self, host='0.0.0.0', port=8000, db_path='metadata.db'):
        self.host = host
        self.port = port
    ...
    def recv_all(self, sock):
        """Read the full incoming request"""
        data = b''
        while True:
            part = sock.recv(4096)
            if not part:
    ...
    def handle_client(self, client_socket):
        try:
            ...
            if action == 'register':
                ...
                response = {'status': 'success'} if success else {'status': 'error', 'message': 'User exists'}
            elif action == 'get_salt':
                salt = self.get_user_salt(request['user_id'])
                response = {'status': 'success', 'salt': salt} if salt else {'status': 'error', 'message': 'User not found'}
                ...
                response = {'status': 'success', 'chunk_ids': chunk_ids}
                ...
                response = {'status': 'success', 'metadata': metadata} if metadata else {'status': 'error', 'message': 'File not found'}
            elif action == 'delete_file':
                deleted = self.delete_file_metadata(request['user_id'], request['filename'])
                response = {'status': 'success'} if deleted else {'status': 'error', 'message': 'File not found'}
    ...
    def start(self):
        ...
        print(f"Metadata server started on {self.host}:{self.port}")
        try:
            while True:
